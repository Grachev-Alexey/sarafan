...
from app.models import Partner, PartnerInfo, ClientsData, DiscountWeightSettings, Category
...
async def get_random_discount(client_data: ClientsData) -> Optional[Tuple[PartnerInfo, bool]]:
    ...
    """
    logging.info(f"Получение случайной скидки с учетом связанного салона, приоритета, истории посещений, соотношения клиентов и приглашенных партнеров самим салоном")
    ...
    available_salons = PartnerInfo.query.filter(
        PartnerInfo.city_id == user_salon_city_id,
        ~PartnerInfo.id.in_(excluded_salon_ids),
        ~PartnerInfo.categories.any(Category.id.in_(excluded_category_ids))
    ).all()
    ...
    if not available_salons:
        logging.error(
            f"Не удалось найти доступные салоны в городе {user_salon_city_id}, кроме взаимодействовавших с клиентом салонов") # Изменено
        return None
    ...
    for salon in available_salons:
        ...
        # Увеличиваем вес в зависимости от количества приглашенных партнеров самим салоном
        partner = Partner.query.filter_by(salon_id=salon.id).first()
        if partner:
    ...
    if weighted_salons:
        ...
        logging.error(f"Не удалось найти доступные салоны в городе {user_salon_city}, кроме категорий {', '.join(excluded_categories)} и взаимодействовавших с клиентом салонов, с учетом соотношения клиентов")
        return None