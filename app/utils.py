...
async def get_random_discount(client_data: ClientsData) -> Optional[Tuple[PartnerInfo, bool]]:
    ...
    Учитывает приоритет салонов и соотношение "привел/получил".
    В первую очередь проверяет наличие приоритетного салона для этого города,
    затем наличие связанного салона.
    """
    logger.info("Получение случайной скидки с учетом связанного салона, приоритета, истории посещений и соотношения клиентов")
    ...
    if not available_salons:
        logger.error(f"Не удалось найти доступные салоны в городе {user_salon_city_id}, кроме взаимодействовавших с клиентом салонов")
        return None
